create or replace package rep_bills_compound2 is
...
/
create or replace package body rep_bills_compound2 is
      ...
      join (select * from (-- фильтр по приоритетному лиц.счету
              select k2.lsk,
               ...
               k2.reu=p_reu and
                ...
                case when p_sel_obj = 2 /*and tp.cd='LSK_TP_MAIN'*/ and k2.prn_num between p_firstNum and p_lastNum then 1 -- по адресам УК, по диапазону pr_num
                 when p_sel_obj != 2 then 1
                else 0 end = 1
                and exists (
                  select * from arch_kart k3 where k3.mg = p_mg and k3.k_lsk_id=k2.k_lsk_id
                     and decode(p_sel_uk, '0', 1, instr(p_sel_uk, ''''||k3.reu||''';', 1)) > 0
                )
               ) a where a.lsk=a.lsk_main) k4 on k4.lsk_main=k.lsk
   where k.mg = p_mg
   order by k.prn_num;
